...
import { sectoralIndices } from '../data/assets';
...
const sectoralIndexSymbols = sectoralIndices.map(index => index.symbol);

const getBenchmarkForSymbol = (symbol: string): { symbol: string; name: string } => {
    if (symbol.endsWith('.NS') || sectoralIndexSymbols.includes(symbol)) {
...
Provide a complete technical analysis based on this data and/or the accompanying chart image.`;
  ...
  // The 'contents' payload is constructed differently for text-only vs. multimodal requests
  // to align with best practices and potentially avoid backend issues with a text-only request
  // being wrapped in a Content object instead of being a simple string.
  ...
  try {
    ...
    if (detailedMessage.includes('JSON') || detailedMessage.includes('json')) {
         throw new Error("The AI returned an invalid analysis format. This can be a temporary issue. Please try your request again.");
    }
    
    if (detailedMessage.includes('Rpc failed due to xhr error')) {
        throw new Error('A network error occurred while contacting the AI service. This could be due to a temporary service issue or a problem with your internet connection. Please try again in a few moments.');
    }
